  ...
  try {
    ...
    sourceFiles.forEach((element) => {
      ...
      if (!exclude.includes(element)) {
        const sourceElementPath = path.join(
          sourceDir,
          element
        )
        const destinationElementPath = path.join(
          projectDir,
          element
        )
        ...
        if (element === 'package.json') {
          ...
          const packageData = JSON.parse(
            fs.readFileSync(sourceElementPath)
          )
          ...
          let webpackConfig = fs.readFileSync(
            sourceElementPath,
            'utf8'
          )
          webpackConfig = webpackConfig.replace(
            /React Boilerplate/g,
            cammelCase(projectName)
          )
          fs.writeFileSync(
            destinationElementPath,
            webpackConfig,
    ...
    // Commit files
    await run(
      'git commit -m "Created React App from @patomation/create-react-app"'
    )
    console.log('Done')
  } catch (error) {
    color.error()
